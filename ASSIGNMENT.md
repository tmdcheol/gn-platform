...
`'use client'` + `useState` 폼. 상태가 필드 두 개뿐이라 라이브러리 없이 충분하지만, 쓰고 싶으면 써도 됩니다.
...
클릭 시 모달 — 오버레이 + 좌우 이동 + `ESC` 닫기. 열려 있는 동안 배경 스크롤 잠금.
직접 구현해도 되고, 접근성(포커스 트랩·`aria-modal`)까지 챙겨주는 라이브러리를 써도 됩니다.
